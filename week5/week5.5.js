...
function onReadComplete(device, pipeline, buffers) {
  ...
  buffers.indexMaterialBuffer = device.createBuffer({
    size: g_drawingInfo.mat_indices.byteLength,
    usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.STORAGE,
  });
  device.queue.writeBuffer(
    buffers.indexMaterialBuffer,
    0,
    g_drawingInfo.mat_indices
  );
  const colors = [];
  const emissions = [];
  ...
  console.log("mat indices:", g_drawingInfo.mat_indices);

  buffers.materialBuffer = device.createBuffer({
    size: Float32Array.BYTES_PER_ELEMENT * colors.length,
    usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.STORAGE,
  });
  device.queue.writeBuffer(buffers.materialBuffer, 0, new Float32Array(colors));
  buffers.emissionBuffer = device.createBuffer({
    size: Float32Array.BYTES_PER_ELEMENT * emissions.length,
    usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.STORAGE,
  });
  device.queue.writeBuffer(
    buffers.emissionBuffer,
    0,
    new Float32Array(emissions)
  );
  console.log(colors);
  buffers.lightBuffer = device.createBuffer({
    size: g_drawingInfo.light_indices.byteLength,
    usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.STORAGE,
  });
  device.queue.writeBuffer(buffers.lightBuffer, 0, g_drawingInfo.light_indices);
  ...
  const bindGroup = device.createBindGroup({
    ...
    entries: [
      ...
      { binding: 4, resource: { buffer: buffers.indexMaterialBuffer } },
      { binding: 5, resource: { buffer: buffers.materialBuffer } },
      { binding: 6, resource: { buffer: buffers.emissionBuffer } },
      { binding: 7, resource: { buffer: buffers.lightBuffer } },
    ],
  });
  return bindGroup;
}
