...
function subdivide_node(node, bbox, level, objects) {
  const TESTS = 4;
  //checks if the number of objectss is less than a threshold or the max depth is reached and creates new node
  ...
  //iterates through different splitting planes and calculates the cost of each split
  //choose the one with the lowest cost
  //divides the objects into left and right child nodes based on the selected splitting plane
  else {
    let left_objects = [];
    let right_objects = [];
    node.left = new Object();
    node.right = new Object();
    //outer loop: iterate over three spatial dimensions and find the best split plane along each of the axes
    //cost for the combination of axis and split position
    var min_cost = 1.0e27;
    for (var i = 0; i < 3; ++i) {
      for (var k = 1; k < TESTS; ++k) {
        ...
        // based on the number of objects on the left and right sides of the split, as well as the areas of the corresponding bounding boxes
        const cost =
...
//building the bsp tree from the input 3D model
//creates an array of AccObj objects for the triangles in the model
//axis aligned bounding box(AABB) is the root node
